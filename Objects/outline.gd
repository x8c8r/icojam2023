...
func update(target_pos:Vector2, position:Vector2) -> void:
	
	visible = GridHelper.is_valid_movement(tilemap, GridHelper.get_cell_pos_in_tilemap(tilemap, position)+direction, GridHelper.get_cell_pos_in_tilemap(tilemap, position))
